...
job_description = data_frame[columns[5]].tolist()
...
with open('result_Comerica Inc.csv', "w", newline='') as f:
    writer = csv.writer(f)
    writer.writerow(header)
    i = 300
    for dict in data:
        i += 1
        csv_list = [i, 'Comerica Inc', dict['url'], 1]
        csv_list += dict['word_counter'].values()
        writer.writerow(csv_list)
        csv_list = [i, 'Comerica Inc', dict['url'], 2]
        csv_list += dict['word_tf_idf'].values()
        writer.writerow(csv_list)
        csv_list = [i, 'Comerica Inc', dict['url'], 3]
